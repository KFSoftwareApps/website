...
export default function BlogCard({ post, index }: BlogCardProps) {
  ...
  return (
    ...
    >
      <Link href={`/blog/read?slug=${post.slug}`} className="relative aspect-video overflow-hidden">
        {post.image ? (
          <Image
            src={post.image}
            alt={post.title}
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <h3 className="text-xl font-bold text-gray-900 mb-3 group-hover:text-blue-600 transition-colors">
          <Link href={`/blog/read?slug=${post.slug}`}>{post.title}</Link>
        </h3>

        <p className="text-gray-600 text-sm line-clamp-3 mb-6 flex-grow">{post.excerpt}</p>

        <Link
          href={`/blog/read?slug=${post.slug}`}
